 ...
 * @author paulo
 */
public class IzbrisiRezervacija extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
         ...
         HttpSession sesija = request.getSession();
            ...
            korisnik=(Korisnik)sesija.getAttribute("korisnik");
           ...
           String dbUrl="jdbc:mysql://localhost:3306/webbioskop";
            ...
            try {
                ...
                String izbrisiF= "delete from rezervacija where rezervacijaID='"+brisanjeF+ "'";
                
                stmt.executeUpdate(izbrisiF);
                stmt.close();
                conn.close();
                
                request.setAttribute("mensa", "Uspesno ste obrisali Rezervaciju");
                
                 if(korisnik.getTip().equals("Admin"))
            {
               request.setAttribute("Admin", korisnik);
               request.getRequestDispatcher("admin.jsp").forward(request, response);
            }
            else if(korisnik.getTip().equals("Menadzer"))
            {
              request.setAttribute("Menadzer", korisnik);
               request.getRequestDispatcher("menadzer.jsp").forward(request, response);
            
            }
            
            else if(korisnik.getTip().equals("Klijent"))
            {
              request.setAttribute("Klijent", korisnik);
